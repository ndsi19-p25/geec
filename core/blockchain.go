...
func (bc *BlockChain) insert(block *types.Block) {
	log.THW("Insert Called", "BlockNum", block.NumberU64());
	...
	thws := bc.hc.GetThwState().(*THWState)
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	log.THW("Write block with state", "Trust rand", block.Header().TrustRand)
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	// Do a sanity check that the provided chain is actually ordered and linked
	log.THW("Insert chain", "start", chain[0].NumberU64(), "end", chain[len(chain)-1].NumberU64())
	...
	for i, block := range chain {
		log.THW("Verifying block", "num", block.NumberU64())
		...
		if err == nil {
			...
			log.THW("Valdiate error", "error", err)
		}
		...
		if err != nil {
			log.THW("WriteBlockWithState Err", err)
			return i, events, coalescedLogs, err
		}
		switch status {
		case CanonStatTy:
			log.THW("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()),
		...
		case SideStatTy:
			log.THW("Inserted forked block", "number", block.Number(), "hash", block.Hash(), "diff", block.Difficulty(), "elapsed",
